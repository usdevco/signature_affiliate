...
class Leads extends Admin_controller
{
    ...
    public function lead($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($id == '') 
            {
                ...
                $lead_id = $id;
                $this->add_lead_clients($lead_id);


                if($this->input->post('email')) 
                { 
                    // $this->lead_welcome_mail($this->input->post('email',TRUE));
                }
    ...
    private function lead_welcome_mail($email)
    {
                
            $this->load->config('email');
            // Simulate fake template to be parsed
            $template = new StdClass();
            $template->message = $this->load->view('emails/welcome.php',$data,TRUE);

            $template->fromname = get_option('companyname') != '' ? get_option('companyname') : 'Signature';
    ...
    private function add_lead_clients($lead_id)
    {
        $tblleads_data = $this->db->where('id', $lead_id)->get('tblleads')->row();
        if($tblleads_data)
        {
            $client_array['company'] = $tblleads_data->name;
            $client_array['phonenumber'] = $tblleads_data->phonenumber;
            // email
            $client_array['country'] = $tblleads_data->country;
            $client_array['city'] = $tblleads_data->city;
            $client_array['zip'] = $tblleads_data->zip;
            $client_array['state'] = $tblleads_data->state;
            $client_array['address'] = $tblleads_data->address;
            $client_array['website'] = $tblleads_data->website;

            $client_array['billing_street'] = $tblleads_data->address;
            $client_array['billing_city'] = $tblleads_data->city;
            $client_array['billing_state'] = $tblleads_data->state;
            $client_array['billing_zip'] = $tblleads_data->zip;
            $client_array['billing_country'] = $tblleads_data->country;

            $client_array['shipping_street'] = $tblleads_data->address;
            $client_array['shipping_city'] = $tblleads_data->city;
            $client_array['shipping_state'] = $tblleads_data->state;
            $client_array['shipping_zip'] = $tblleads_data->zip;
            $client_array['shipping_country'] = $tblleads_data->country;

            $client_array['datecreated'] = date("Y-m-d H:i:s");
            $client_array['active'] = 1;
            $client_array['leadid'] = $tblleads_data->id;

            $client_array['default_currency'] = 0;
            $client_array['show_primary_contact'] = 0;
            $client_array['addedfrom'] = get_staff_user_id();
            $client_array['from_lead'] = 1;

            $this->db->insert('tblclients', $client_array);

            $clientid = $this->db->insert_id();
            $contacts_contant = array();

            $contacts_contant['userid'] = $clientid;
            $contacts_contant['is_primary'] = 1;
            $contacts_contant['firstname'] = $tblleads_data->name;
            $contacts_contant['lastname'] = ' ';
            $contacts_contant['email'] = $tblleads_data->email;
            $contacts_contant['phonenumber'] = $tblleads_data->phonenumber;
            $contacts_contant['title'] = 'Mr';
            $contacts_contant['datecreated'] = date("Y-m-d H:i:s");
            $contacts_contant['password'] = NULL;
            $contacts_contant['new_pass_key'] = NULL;
            $contacts_contant['new_pass_key_requested'] = NULL;
            $contacts_contant['last_ip'] = NULL;
            $contacts_contant['last_login'] = NULL;
            $contacts_contant['last_password_change'] =NULL;
            $contacts_contant['active'] = 1;
            $contacts_contant['profile_image'] = NULL;
            $contacts_contant['direction'] = NULL;
            $contacts_contant['invoice_emails'] = 1;
            $contacts_contant['estimate_emails'] = 1;
            $contacts_contant['contract_emails'] = 1;
            $contacts_contant['task_emails'] = 1;
            $contacts_contant['project_emails'] = 1;
            $contacts_contant['ticket_emails'] = 1;
            $this->db->insert('tblcontacts', $contacts_contant);
