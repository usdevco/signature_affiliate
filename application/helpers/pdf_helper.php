...
function pdf_logo_url()
{
    ...
    // $custom_pdf_logo_image_url = base_url().'uploads/2017/05/SIWD_Logo.png';
    $custom_pdf_logo_image_url = $custom_pdf_logo_image_url ? $custom_pdf_logo_image_url : base_url().'uploads/2017/05/SIWD_Logo.png';
    // p($custom_pdf_logo_image_url);
...
function estimate_attachment_pdf($estimate_attacchment)
{
    $GLOBALS['estimate_atach_pdf'] = $estimate_attacchment;
    $CI                      = & get_instance();
    $CI->load->library('pdf');
    ...
    if (!file_exists(APPPATH . 'libraries/Contract_pdf.php')) {
        ...
        include_once(APPPATH . 'libraries/estimate_atach.php');
        $pdf = new Contract_pdf($formatArray['orientation'], 'mm', $formatArray['format'], true, 'UTF-8', false, false);
    }
    ...
    $pdf->SetTitle($estimate_attacchment->name);
    $pdf->SetAuthor(get_option('company'));
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->AddPage($formatArray['orientation'], $formatArray['format']);
    if ($CI->input->get('print') == 'true') {
    ...
    # Don't remove these lines - important for the PDF layout
    // Add <br /> tag and wrap over div element every image to prevent overlaping over text
    $estimate_attacchment->content = preg_replace('/(<img[^>]+>(?:<\/img>)?)/i', '<div>$1</div>', $estimate_attacchment->content);
    // Add cellpadding to all tables inside the html
    $estimate_attacchment->content = preg_replace('/(<table\b[^><]*)>/i', '$1 cellpadding="4">', $estimate_attacchment->content);
    // Remove white spaces cased by the html editor ex. <td>  item</td>
    $estimate_attacchment->content = preg_replace('/[\t\n\r\0\x0B]/', '', $estimate_attacchment->content);
    $estimate_attacchment->content = preg_replace('/([\s])\1+/', ' ', $estimate_attacchment->content);

    // Tcpdf does not support float css we need to adjust this here
    $estimate_attacchment->content = str_replace('float: right', 'text-align: right', $estimate_attacchment->content);
    $estimate_attacchment->content = str_replace('float: left', 'text-align: left', $estimate_attacchment->content);
    // Image center
    $estimate_attacchment->content = str_replace('margin-left: auto; margin-right: auto;', 'text-align:center;', $estimate_attacchment->content);

    if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/estimate_atach_pdf.php')) {
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/estimate_atach_pdf.php');
    } else {
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/estimate_atach_pdf.php');
    }
