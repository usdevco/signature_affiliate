...
class Projects_model extends CRM_Model
{
            ...
            if ($project) {
                ...
                foreach ($settings as $key => $setting) {
                    if ($setting['name'] == 'available_features') {
                        ...
                        $available_features = $setting['value'] ? unserialize($setting['value']) : $setting['value'];
                    ...
                    foreach ($tabs_flatten as $tab) {
                        if (!in_array($tab, $settings_available_features)) {
                            if ($available_features_index) {
                                ...
                                $tmp                                 = $current_available_features_settings['value'] ? unserialize($current_available_features_settings['value']) : NULL;
                                $tmp[$tab]                           = 1;
    ...
    public function get_tasks($id, $where = [], $apply_restrictions = false, $count = false)
    {
        ...
        if (!is_client_logged_in() && is_staff_logged_in()) {
            $select .= ',(SELECT staffid FROM tblstafftaskassignees WHERE taskid=tblstafftasks.id AND staffid=' . get_staff_user_id() . ' LIMIT 1) as current_user_is_assigned';
        }
        ...
        $this->db->order_by('tblstafftasks.milestone_order', 'asc');
        $this->db->where($where);
    ...
    public function get_progress_step($id,$progress_step)
    {

        $this->db->select('tblstafftasks.id,tblstafftasks.rel_id,name,startdate,duedate,progress_step,progress_step,custom_input_value,is_complete,datefinished,dateadded,tblstafftasks.status as task_status,(select input_type from tblcustomtask where tblstafftasks.custom_task_id = tblcustomtask.id) as input_type,(select input_type_label from tblcustomtask where tblstafftasks.custom_task_id = tblcustomtask.id) as input_type_label,(select input_type_help_text from tblcustomtask where tblstafftasks.custom_task_id = tblcustomtask.id) as input_type_help_text,(select dropdown_options from tblcustomtask where tblstafftasks.custom_task_id = tblcustomtask.id) as dropdown_options,(select role_id from tblcustomtask where tblstafftasks.custom_task_id = tblcustomtask.id) as role_id');        
        $this->db->where('rel_id',$id);
        $this->db->where('progress_step',$progress_step);
        return $this->db->get('tblstafftasks')->result();
        
    }

    public function get_assign_staff($task_id)
    {
        $this->db->select('tblstaff.staffid,tblstaff.phonenumber,tblstaff.email,tblstaff.firstname,tblstaff.lastname,tblstaff.profile_image');
    ...
    public function get_custom_task_by_progress_step($progress_step_slug)
    {

        $this->db->select('tblcustomtask.*,tbltaskstep.title as task_step_name,tbltaskstep.slug as task_step_slug,
            (select name from tblroles where tblroles.roleid = tblcustomtask.role_id) as user_role,
            (select mail_templet from tblmailtemplet where tblmailtemplet.id = tblcustomtask.client_mail_templet) as client_mail_templet_name,
            (select mail_templet from tblmailtemplet where tblmailtemplet.id = tblcustomtask.staff_mail_templet) as staff_mail_templet_name');
        $this->db->join('tbltaskstep','tbltaskstep.id = tblcustomtask.task_step_id');
        $this->db->where('tbltaskstep.slug',$progress_step_slug);
