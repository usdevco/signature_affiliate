...
class Tasks_model extends CRM_Model
{
    ...
    public function get_task_step_by_slug($task_step_slug)
    {
        $this->db->where('slug',$task_step_slug);
        return $this->db->get('tbltaskstep')->row();
    }
    ...
    {
        $this->db->select("tblclients.*, (select email from tblcontacts where tblcontacts.userid = tblclients.userid) as email");
    ...
    public function get_task_by_id($task_id)
    {
        ...
        $this->db->select('rel_type,rel_id,name,visible_to_client,status,addedfrom,mail_template');
        $this->db->where('id', $task_id);
        $task = $this->db->get('tblstafftasks')->row();
    ...
    public function get_hoa_task_by_id($task_id)
    {

        $this->db->select('rel_type,rel_id,name,visible_to_client,status,addedfrom,mail_template');
        $this->db->where('id', $task_id);
        $this->db->where('mail_template', 'hoa-approved');
        $task = $this->db->get('tblstafftasks')->row();
...
*/
    ...
    public function get_tasks_assign_by_staff_id($staff_id, $task_aval_id_array)
    {
            $this->db->select('tblstafftasks.id, tblstafftasks.name,');
            $this->db->join('tblstafftasks', 'tblstafftaskassignees.taskid = tblstafftasks.id','left');
            $this->db->where('tblstafftaskassignees.staffid',$staff_id);
            $this->db->where('tblstafftasks.mail_template','hoa-approved');
            if($task_aval_id_array)
            {
                $this->db->where_not_in('tblstafftasks.id',$task_aval_id_array);
            }
            return $this->db->get('tblstafftaskassignees')->result();
    }
    public function all_role_array()
    {
        return $this->db->get('tblroles')->result();
    }

    public function custom_task_array()
    {
        return $this->db->get('tblcustomtask')->result();
    }

    public function task_step_array()
    {
        return $this->db->get('tbltaskstep')->result();
    }

    public function task_mail_templets()
    {
        return $this->db->get('tblmailtemplet')->result();
    }

    public function get_tasks_by_task_id($task_id)
    {
        return $this->db->where('id',$task_id)->get('tblstafftasks')->row_array();
    }

    public function get_staff_by_id($staff_id)
    {
         return $this->db->where('staffid',$staff_id)->where('role',2)->get('tblstaff')->row();
    }

    public function insert_hoa_detail($data)
    {
        $this->db->insert('tbl_task_hoa_details', $data);
        $insert_id = $this->db->insert_id();
        return  $insert_id;
    }

    public function insert_check_list($data)
    {
        $this->db->insert('tblchecklist', $data);
        $insert_id = $this->db->insert_id();
        return  $insert_id;
    }

    public function insert_custom_task($data)
    {
        $this->db->insert('tblcustomtask', $data);
        $insert_id = $this->db->insert_id();
        return  $insert_id;
    }

    public function update_check_list($check_list_content,$check_list_id)
    {
        $this->db->where('id',$check_list_id)->update('tblchecklist',$check_list_content);
        return $this->db->affected_rows();
    }

    public function update_custom_task_data($custom_task_content,$custom_task_id)
    {
        $this->db->where('id',$custom_task_id)->update('tblcustomtask',$custom_task_content);
        return $this->db->affected_rows();
    }

    public function get_check_list_data_by_id($check_list_id)
    {
        return $this->db->where('id',$check_list_id)->get('tblchecklist')->row();
    }

    public function get_task_hoa_by_taskid_user_id($task_id,$staff_id)
    {
        return $this->db->where('staff_id',$staff_id)->where('task_id',$task_id)->get('tbl_task_hoa_details')->row();
    }

    public function get_task_aval_hoa_by_user_id($staff_id)
    {
        $hoa_detail_data = $this->db->where('staff_id',$staff_id)->get('tbl_task_hoa_details')->result();

        $hoa_detail_id_array = array();

        foreach ($hoa_detail_data as $hoa_array) 
        {
            $hoa_detail_id_array[] = $hoa_array->task_id;
        }

        return $hoa_detail_id_array;
    }

    public function get_task_hoa_detail_by_id($hoa_task_detail_id)
    {
        return $this->db->where('id',$hoa_task_detail_id)->get('tbl_task_hoa_details')->row();
    }

    public function update_hoa_detail($hoa_content, $hoa_task_detail_id)
    {
        $this->db->where('id',$hoa_task_detail_id)->update('tbl_task_hoa_details',$hoa_content);
        return $this->db->affected_rows();
    }
}
