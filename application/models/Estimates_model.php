...
class Estimates_model extends CRM_Model
{
    ...
    public function do_kanban_query($status, $search = '', $page = 1, $sort = [], $count = false)
    {
        ...
        $this->db->from('tblestimates');
        $this->db->join('
            ', 'tblclients.userid = tblestimates.clientid', 'left');
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            // $this->add_new_project($insert_id);
    ...
    public function add_new_project($id)
    {
        ...
        $project_array['name'] = "Project for Estimate #".$estimated_data->id;
        $project_array['description'] = "";
        $project_array['status'] = 1;
        $project_array['clientid'] = $estimated_data->clientid;
        ...
        {
            $task_to_be_assigned = [
                        array('title' => 'Final Measurements & Diagram (Uploaded)'), 
                        array('title' => 'Signed Permit Application (Uploaded)'), 
                        array('title' => 'Schedule Install (Email Tigger That The Order Has Arrived)'), 
                        array('title' => 'Inspect Project'), 
                        array('title' => 'Signed Client Walk Through (Uploaded)'), 
                        array('title' => 'Final Inspection (Uploaded) (Email Trigger That The Permit Has Been Closed & Job Is Complete)'), 
                        array('title' => 'HOA Approval (If Needed) (Email Trigger That The HOA Has Been Approved)'), 
                        array('title' => 'Entering & Placing Order (Uploaded) (Email Trigger To Notify Client That Order Has Been Placed)'), 
                        array('title' => 'Submit For Engineering If Needed'), 
                        array('title' => 'Confirming Final Measurement Frame Color Glass Color & Operation Of All Openings'), 
                        array('title' => 'Prep Permit Application & Send To Project Manager'), 
                        array('title' => 'Expedite Permit Package To The City (Email Trigger That The Permit Has Been Submitted)'), 
                        array('title' => 'Print Permit Package Once Approved (Email Trigger That The City Has Approved The Project)')
                        ];

            foreach ($task_to_be_assigned as  $task_value_array) 
            {
                $this->create_custom_task($task_value_array, $project_id, $clientid);
            }
    ...
    public function create_custom_task($task_value_array, $project_id, $clientid)
    {
            ...
            $tsk_id = $this->db->insert_id();
             ...
             $tblstaff_user = $this->db->get('tblstaff')->result();
