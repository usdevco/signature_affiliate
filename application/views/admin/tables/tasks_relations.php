...
$aColumns = [
    ...
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tbltags_in JOIN tbltags ON tbltags_in.tag_id = tbltags.id WHERE rel_id = tblstafftasks.id and rel_type="task" ORDER by tag_order ASC LIMIT 1) as tags',
    'priority',
    'rel_id',
];
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    'tblstafftasks.id','status',
    'billed',
    '(SELECT staffid FROM tblstafftaskassignees WHERE taskid=tblstafftasks.id AND staffid=' . get_staff_user_id() . ' LIMIT 1) as is_assigned',
     get_sql_select_task_assignees_ids() . ' as assignees_ids',
     '(SELECT MAX(id) FROM tbltaskstimers WHERE task_id=tblstafftasks.id and staff_id=' . get_staff_user_id() . ' and end_time IS NULL LIMIT 1) as not_finished_timer_by_current_staff',
       '(SELECT staffid FROM tblstafftaskassignees WHERE taskid=tblstafftasks.id AND staffid=' . get_staff_user_id() . ' LIMIT 1) as current_user_is_assigned',
        '(SELECT CASE WHEN addedfrom=' . get_staff_user_id() . ' AND is_added_from_contact=0 THEN 1 ELSE 0 END LIMIT 1) as current_user_is_creator',
        'progress_step',
]);
...
*/
    ...
    $row[] = "<a href='".admin_url('tasks/task_progress/').$aRow['rel_id']."/".$aRow['progress_step']."' class='btn btn-link'><i class='glyphicon glyphicon-eye-open'></i></a>";
