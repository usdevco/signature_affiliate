...
$result = data_tables_init(
    ...
    [
        ...
        '(SELECT id FROM tbl_task_hoa_details WHERE task_id=tblstafftasks.id AND staff_id=' . get_staff_user_id() . ') as tbl_task_hoa_id',
    ]
);
...
*/
    ...
    $danger = $aRow["tbl_task_hoa_id"] ? 'text-primary' : 'text-danger';
   
    $staff_role = staff_role(get_staff_user_id());

    if($staff_role == 2)
    {     
        if($aRow["mail_template"] == 'hoa-approved')
        {
             $hoa_detail_url = $aRow["tbl_task_hoa_id"] ? base_url('admin/tasks/view_hoa_details_for_task/').$aRow["tbl_task_hoa_id"] : base_url('admin/tasks/hoa_details_for_task/').$aRow["id"];

            $row[] = '<a href="'.$hoa_detail_url.'"> <i class="fa fa-eye '.$danger.'"></i></a>';
        }
        else
        {
            $row[] = '<span> - </span>';
